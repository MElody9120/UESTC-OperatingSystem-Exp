...
class Processor:
    ...
    def create_process(self, pid, priority):
        # 首先判断该进程是否已经存在
        if not len(self._running_list) == 0:
            if pid in [x.get_pid() for x in self.get_process_list()]:
                print("Create process failed, process '" + pid + "' existed!")
                return
        # 新创建的进程将会插入到RL队列的末尾
    ...
    def delete_process(self, resource, pid, time):
        ...
        if len(resources) != 0:
            for i in range(len(resources)):
                self.release_resource(process=process,
                                      resource=resource,
                                      rid=resources[0]['rid'],
                                      release_status=resources[0]['status'])
    ...
    def release_resource(self, resource, rid, release_status, process=None):
        ...
        # 如果已分配资源大于等于要求释放资源，则释放资源，并修改该进程状态
